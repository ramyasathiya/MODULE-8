...
Define a function height(Node* n) to return the height of a node.

Define a function getBalance(Node* n) to calculate the balance factor: balance = height(n->left) - height(n->right)

Define newNode(int val) to create and initialize a new node.

Define rotations:

rightRotate(Node* y) → Perform RR rotation.
leftRotate(Node* x) → Perform LL rotation.
Define insert(Node* node, int key) that:
...
Define preOrder(Node* root) to display the traversal of the tree.
...
```
## Output:
<img width="877" height="513" alt="image" src="https://github.com/user-attachments/assets/529541c5-005f-49d3-ac1d-4b5ebf942ac3" />
